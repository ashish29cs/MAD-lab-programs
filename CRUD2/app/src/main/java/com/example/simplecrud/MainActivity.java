    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        create.setOnClickListener(v -> {
            ...
            if(name.getText().toString().length()!=0 && model.getText().toString().length()!=0){
                ...
                if (checkInsertData) {
                    Toast.makeText(MainActivity.this, "New Car Added Successfully",
                ...
                Toast.makeText(MainActivity.this, "Fields cant be empty",
        ...
        update.setOnClickListener(v -> {
            ...
            if (id.getVisibility() == View.VISIBLE) {
                if (id.getText().toString().length()!=0 && name.getText().toString().length() != 0 && model.getText().toString().length() != 0) {
                    Boolean check =
                            db.updateData(Integer.parseInt(id.getText().toString()),name.getText().toString(),
                                    model.getText().toString());
                    if (check) {
                        Toast.makeText(MainActivity.this, "Details Updated",
                                Toast.LENGTH_LONG).show();
                        id.setText("");
                        id.setVisibility(View.INVISIBLE);
                        name.setText("");
                        ...
                        Toast.makeText(MainActivity.this, "No car name found",
                                Toast.LENGTH_LONG).show();
                    }
                }else{
                    Toast.makeText(MainActivity.this, "Fields cant be empty",
        ...
        delete.setOnClickListener(v -> {
            ...
            if (id.getVisibility() == View.VISIBLE) {
                if (id.getText().toString().length() != 0) {
                    ...
                    if (check) {
                        Toast.makeText(getApplicationContext(), "Car Deleted",
                                Toast.LENGTH_LONG).show();
                        id.setText("");
                        id.setVisibility(View.INVISIBLE);
                    } else {
                        Toast.makeText(MainActivity.this, "Car not Deleted ",
                                Toast.LENGTH_SHORT).show();
                    }
                }else{
                    Toast.makeText(MainActivity.this, "Please enter the car id",
                            Toast.LENGTH_SHORT).show();
                }
            }else{
                id.setVisibility(View.VISIBLE);
